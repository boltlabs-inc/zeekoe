...
impl Command for Show {
    async fn run(self, config: Config) -> Result<(), anyhow::Error> {
        ...
        table.add_row(vec![Cell::new("Channel ID"), Cell::new(details.0)]);
        table.add_row(vec![Cell::new("Status"), Cell::new(details.1)]);
        table.add_row(vec![Cell::new("Contract ID"), Cell::new(details.2)]);
        table.add_row(vec![
            Cell::new("Merchant Deposit"),
            Cell::new(details.3.into_inner()),
        ]);
        table.add_row(vec![
            Cell::new("Customer Deposit"),
            Cell::new(details.4.into_inner()),
        ]);
